...
const TagList = ({ items, className }) => (
  <div className="flex flex-wrap gap-2">
    {items.map((item, i) => (
      <span key={i} className={`text-xs font-semibold px-3 py-1.5 rounded-xl border ${className}`}>
        {item}
      </span>
    ))}
  </div>
...
function CourseCard({ course }) {
  ...
  const toggleModule = useCallback((i) => {
    setActiveModule(prev => prev === i ? null : i);
  }, []);

  return (
    <div className={`rounded-3xl border-2 transition-all duration-300 overflow-hidden bg-white
      ${open
        ? `${c.ring} ring-1 shadow-2xl ${c.glow}`
    ...
    >
      ...
      >
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300
          ${open ? `${c.bg} ${c.text}` : "bg-gray-100 text-gray-400"}`}>
        ...
        <div className="px-6 pb-6 pt-2 space-y-5">
          ...
          {course.projects.length > 0 && (
            <div>
              ...
              <div className="mt-3">
                <TagList items={course.projects} className={c.tool} />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div className="mt-3">
              <TagList items={course.tools} className={c.tool} />
            </div>
          </div>
        </div>
      )}
...
const STATS = [["8", "Courses"], ["80+", "Modules"], ["50+", "Projects"], ["100%", "Job Ready"]];
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-8 py-16 sm:py-24 text-center">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-blue-500 leading-tight tracking-tight mb-4">
            Explore Our Full
            <span className="block">syllabus</span>
          </h1>
